...
// Just checks the connection to the SMTP relay server
...
async fn execute<T: AsyncTransport<Error: Debug> + Send + Sync + 'static>(
    timestamp: impl AsRef<Path>,
    database: ConnectOptions,
    incidents: impl IntoIterator<Item = Incident>,
    username: Option<String>,
    ...
    let users: Vec<_> = fetch_users(&sea_orm::Database::connect(database).await?).await?;
...
async fn fetch_users(db: &impl sea_orm::ConnectionTrait) -> Result<Vec<Subscriber>> {
...
// Pre-creates as mush of the message as I can before copying it to be finished for each user
...
mod test {
    ...
    async fn test_fetch_users() {
        ...
        let station_table = schema.create_table_from_entity(Station);
        let user_station_table = schema.create_table_from_entity(UserStation);

        db.execute(backend.build(&user_table)).await.unwrap();
        db.execute(backend.build(&station_table)).await.unwrap();
        db.execute(backend.build(&user_station_table))
            .await
            .unwrap();

        let users = [
            user::ActiveModel {
                email: ActiveValue::Set(Address::new("alice", "email.com").unwrap().to_string()),
                ..Default::default()
            },
            user::ActiveModel {
                email: ActiveValue::Set(Address::new("bob", "email.com").unwrap().to_string()),
                ..Default::default()
            },
            user::ActiveModel {
                email: ActiveValue::Set(Address::new("charlie", "email.com").unwrap().to_string()),
                ..Default::default()
            },
        ];
        ...
        let stations = [
            station::ActiveModel {
                name: ActiveValue::Set(String::from("foo")),
                ..Default::default()
            },
            station::ActiveModel {
                name: ActiveValue::Set(String::from("bar")),
                ..Default::default()
            },
            station::ActiveModel {
                name: ActiveValue::Set(String::from("baz")),
                ..Default::default()
            },
        ];
        ...
        let subscribers = fetch_users(&db).await.unwrap();
        let expected = [
            Subscriber {
                email: Address::new("alice", "email.com").unwrap(),
                stations: vec![String::from("foo"), String::from("bar")],
            },
            Subscriber {
                email: Address::new("bob", "email.com").unwrap(),
                stations: vec![],
            },
            Subscriber {
                email: Address::new("charlie", "email.com").unwrap(),
                stations: vec![String::from("baz")],
            },
        ];
